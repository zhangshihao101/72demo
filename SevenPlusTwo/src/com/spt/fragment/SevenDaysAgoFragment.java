...
public class SevenDaysAgoFragment extends Fragment {

	private View view;
	private Context mContext;
	private LineChartView line_chart_7_count, line_chart_7_sum;

	private static ProgressDialog dialog;
	private Intent getDays;
	private HashMap<String, String> params;// 参数集合
	private boolean isServiceRunning = false;
	private BroadcastReceiver brHttp_getSevenDays;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		view = inflater.inflate(R.layout.fragment_sevendaysago, null);
	...
	private void initLineChartCount() {
		...
		line_chart_7_count.setInteractive(true);
		line_chart_7_count.setZoomType(ZoomType.HORIZONTAL); // 缩放类型，水平
		line_chart_7_count.setMaxZoom((float) 3);// 缩放比例
		line_chart_7_count.setLineChartData(data);
		line_chart_7_count.setVisibility(View.VISIBLE);

		Viewport vcount = new Viewport(line_chart_7_count.getMaximumViewport());
		vcount.left = 0;
		vcount.right = 7;
		line_chart_7_count.setCurrentViewport(vcount);
	}
	...
	private void initLineChartSum() {
		...
		LineChartData dataSum = new LineChartData();
		dataSum.setLines(lineSum);
		...
		axisX.setMaxLabelChars(7); // 最多几个X轴坐标，意思就是你的缩放让X轴上数据的个数7<=x<=mAxisValues.length
		axisX.setValues(mAxisXValues); // 填充X轴的坐标名称
		dataSum.setAxisXBottom(axisX); // x 轴在底部
		// data.setAxisXTop(axisX); //x 轴在顶部
		axisX.setHasLines(true); // x 轴分割线
		...
		axisY.setName("");// y轴标注
		axisY.setTextSize(11);// 设置字体大小
		dataSum.setAxisYLeft(axisY); // Y轴设置在左边
		// data.setAxisYRight(axisY); //y轴设置在右边

		line_chart_7_sum.setInteractive(true);
		line_chart_7_sum.setZoomType(ZoomType.HORIZONTAL); // 缩放类型，水平
		line_chart_7_sum.setMaxZoom((float) 3);// 缩放比例
		line_chart_7_sum.setLineChartData(dataSum);
		line_chart_7_sum.setVisibility(View.VISIBLE);

		Viewport vcount = new Viewport(line_chart_7_sum.getMaximumViewport());
		vcount.left = 0;
		vcount.right = 7;
		line_chart_7_sum.setCurrentViewport(vcount);
	}
	...
	private void intiView() {
		...
		line_chart_7_count = (LineChartView) view.findViewById(R.id.line_chart_7_count);
		line_chart_7_sum = (LineChartView) view.findViewById(R.id.line_chart_7_sum);

		dialog = ProgressDialog.show(mContext, "请稍候。。。", "获取数据中。。。", true);
		dialog.dismiss();

		params = new HashMap<String, String>(); // 调用接口参数
		brHttp_getSevenDays = new LoginBroadCastReceiver();
		getDays = new Intent(mContext, MyHttpPostService.class);
		getDays.setAction(MyConstant.HttpPostServiceAciton);

		params.clear();
		params.put("externalLoginKey", Localxml.search(mContext, "externalloginkey"));
		params.put("startDate", MyDate.getSevenAgoDate());
		params.put("endDate", MyDate.getDate());

		String uri = MtsUrls.base + MtsUrls.get_daysago;
		dialog.show();
		getDays.putExtra("uri", uri);
		getDays.putExtra("param", params);
		getDays.putExtra("type", "seven");
		isServiceRunning = true;
		mContext.startService(getDays);
	}
	...
	@Override
	public void onStop() {
		mContext.unregisterReceiver(brHttp_getSevenDays);
		if (isServiceRunning) {
			isServiceRunning = false;
			mContext.stopService(getDays);
		}
		super.onStop();
	}
	...
	private void registMyBroadcastRecevier() {
		...
		mContext.registerReceiver(brHttp_getSevenDays, filterHttp);
	}
